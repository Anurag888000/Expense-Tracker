...
let isProcessingRecurringTransactions = false

export async function processRecurringTransactions() {
  if (isProcessingRecurringTransactions) {
    return
  }

  isProcessingRecurringTransactions = true
  ...
  try {
    ...
    for (const template of dueTransactions) {
      ...
      // 2. Calculate the *next* next_date using UTC to avoid timezone off-by-one errors
      const currentNextDate = new Date(`${template.next_date}T00:00:00Z`)
      let bumpedDate = new Date(currentNextDate.getTime())

      if (template.frequency === 'weekly') {
        bumpedDate.setUTCDate(bumpedDate.getUTCDate() + 7)
      } else if (template.frequency === 'monthly') {
        bumpedDate.setUTCMonth(bumpedDate.getUTCMonth() + 1)
      } else if (template.frequency === 'yearly') {
        bumpedDate.setUTCFullYear(bumpedDate.getUTCFullYear() + 1)
      }
    ...
    isProcessingRecurringTransactions = false
  }
}
